...
public class CardRestService {
	...
	public String getDeckAnalysis(@QueryParam("deckcode") String code, @QueryParam("queries") String queries)
		...
		try {
			...
			Map<String, Map<Card, Integer>> categorymap = new LinkedHashMap<>();
			...
			JSONObject data = readJsonFromUrl(
					"https://shadowverse-portal.com/api/v1/deck/import?format=json&deck_code=" + code)
							.getJSONObject("data");
			String deckhash = data.getString("hash");
			String deckclass = getCSS(data.getInt("clan"));

			return produceHTML(deck, categorymap, boardBuildingCount, removalCount, resourceCount, followerCount,
					spellCount, amuletCount, neutralCount, zerocost, onecost, twocost, threecost, fourcost, fivecost,
					sixcost, sevencost, eightcost, ninecost, tencost, deckhash, deckclass);
		} catch (Exception e) {
			e.printStackTrace();
	...
	private String produceHTML(ArrayList<Card> deck, Map<String, Map<Card, Integer>> categorymap,
			int boardBuildingCount, int removalCount, int resourceCount, int followerCount, int spellCount,
			int amuletCount, int neutralCount, int zerocost, int onecost, int twocost, int threecost, int fourcost,
			int fivecost, int sixcost, int sevencost, int eightcost, int ninecost, int tencost, String deckhash,
			String deckclass) throws IOException {
		...
		htmlString = htmlString.replace("$cardcategory",
				"[" + boardBuildingCount + ", " + removalCount + ", " + resourceCount + "]");
		htmlString = htmlString.replace("$cardcosts",
				"[" + zerocost + ", " + onecost + ", " + twocost + ", " + threecost + ", " + fourcost + ", "
						+ fivecost + ", " + sixcost + ", " + sevencost + ", " + eightcost + ", " + ninecost + ","
						+ tencost + "]");
		htmlString = htmlString.replace("$cardcostline", "[" + (zerocost + onecost) + ", "
				+ (twocost + zerocost + onecost) + ", " + (threecost + twocost + zerocost + onecost) + ", "
				+ (fourcost + threecost + twocost + zerocost + onecost) + ", "
				+ (fivecost + fourcost + threecost + twocost + zerocost + onecost) + ", "
				+ (sixcost + fivecost + fourcost + threecost + twocost + zerocost + onecost) + ", "
				+ (sevencost + sixcost + fivecost + fourcost + threecost + twocost + zerocost + onecost) + ", "
				+ (eightcost + sevencost + sixcost + fivecost + fourcost + threecost + twocost + zerocost + onecost)
				+ ", "
				+ (ninecost + eightcost + sevencost + sixcost + fivecost + fourcost + threecost + twocost + zerocost
						+ onecost)
				+ "," + (tencost + ninecost + eightcost + sevencost + sixcost + fivecost + fourcost + threecost
						+ twocost + zerocost + onecost)
				+ "]");
		htmlString = htmlString.replace("$cardtypes",
				"[" + followerCount + ", " + spellCount + ", " + amuletCount + "]");
		htmlString = htmlString.replace("$cardclasses",
				"[" + (deck.size() - neutralCount) + ", " + neutralCount + "]");
		return htmlString.replace("$cardcategories", generateCardCategoryDivs(categorymap));
	}

	private Map<String, Map<Card, Integer>> getQueryCategories(Map<String, Map<Card, Integer>> categorymap,
			ArrayList<Card> deck, String s) {
		...
		if (s.contains("Machina")) {
			categorymap.put("Machina", service.getMachinaCardsFromDeck(deck));
		}
	...
	private String generateCardDiv(Card card, int count) {
		...
		body = body.replace("$card_name", card.getCardName());
		body = body.replace("$amount", String.valueOf(count));
		body = body.replace("$card_id", String.valueOf(card.getCardID()));
		return body;
	}
	 ...
	 * @Path("/spells")
	 * 
	 * @Produces("application/json") public String getSpellCards
	 ...
	 * @Path("/amulets")
	 * 
	 * @Produces("application/json") public String getAmuletCards
	 * (@QueryParam("code") String code) {
	 * ArrayList<Card>deck=getDeckFromCode(code);
	 * 
	 ...
	 * @Path("/followers")
	 * 
	 * @Produces("application/json") public String getFollowerCards
	 * (@QueryParam("code") String code) {
	 * ArrayList<Card>deck=getDeckFromCode(code);
	 * 
	 ...
	 * @Path("/removal")
	 * 
	 * @Produces("application/json") public String getRemovalCards
	 * (@QueryParam("code") String code) {
	 * ArrayList<Card>deck=getDeckFromCode(code);
	 * 
	 ...
	 * @Path("/aoeremoval")
	 * 
	 * @Produces("application/json") public String getAoERemovalCards
	 * (@QueryParam("code") String code) {
	 * ArrayList<Card>deck=getDeckFromCode(code);
	 * 
	 * JsonArrayBuilder
	 * jab=getJsonFromDeck(service.getAoERemovalCardsFromDeck(deck)); return
	 * jab.build().toString();
	 * 
	 * }
	 * 
	 ...
	 * @Path("/burn")
	 * 
	 * @Produces("application/json") public String getBurnRemovalCards
	 * (@QueryParam("code") String code) {
	 * ArrayList<Card>deck=getDeckFromCode(code);
	 * 
	 ...
	 * @Path("/early")
	 * 
	 * @Produces("application/json") public String getEarlyCards
	 * (@QueryParam("code") String code) {
	 * ArrayList<Card>deck=getDeckFromCode(code);
	 * 
	 ...
	 * @Path("/mid")
	 * 
	 * @Produces("application/json") public String getMidCards (@QueryParam("code")
	 * String code) { ArrayList<Card>deck=getDeckFromCode(code);
	 * 
	 ...
	 * @Path("/late")
	 * 
	 * @Produces("application/json") public String getLateCards (@QueryParam("code")
	 * String code) { ArrayList<Card>deck=getDeckFromCode(code);
	 * 
	...
	private ArrayList<Card> getDeckFromCode(String code){
		...
		JSONObject deckdata = readJsonFromUrl(
				"https://shadowverse-portal.com/api/v1/deck?format=json&lang=EN&hash=" + deckhash).getJSONObject("data")
						.getJSONObject("deck");
	...
	private JsonArrayBuilder getJsonFromDeck(ArrayList<Card> deck) {
		...
		for (Card c : deck) {
			...
			job.add("card_id", c.getCardID());
			job.add("foil_card_id", c.getFoilCardID());
			job.add("card_set_id", c.getCardSetID());
			job.add("card_name", c.getCardName());
			job.add("is_foil", c.getIsFoil());
			job.add("char_type", c.getCharType());
			job.add("clan", c.getClan());
			job.add("tribe_name", c.getTribeName());
			job.add("skill_disc", c.getSkillDisc());
			job.add("evo_skill_disc", c.getEvoSkillDisc());
			job.add("cost", c.getCost());
			job.add("atk", c.getAtk());
			job.add("life", c.getLife());
			job.add("evo_atk", c.getEvoAtk());
			job.add("evo_life", c.getEvoLife());
			job.add("rarity", c.getRarity());
			job.add("get_red_ether", c.getGetRedEther());
			job.add("use_red_ether", c.getUseRedEther());
			job.add("description", c.getDescription());
			job.add("evo_description", c.getEvoDescription());
			job.add("cv", "-");
			job.add("base_card_id", c.getBaseCardID());
			if (c.getTokens() == null) {
				job.add("tokens", "-");
			} else {
				job.add("tokens", c.getTokens());
			}
			job.add("normal_card_id", c.getNormalCardID());
			job.add("format_type", c.getFormatType());
			job.add("restricted_count", c.getRestrictedCount());
			results.add(job);
		}
		return results;
	}
